...
Returns:
An MCC tree object with parsed metadata, node heights, and tip details.
Suggested Enhancements:
•	Optionally highlight branches by genotype from a metadata column.
•	Convert tree layout (rectangular, radial, cladogram).
________________________________________
2. Trait Management
...
hpdLevels	integer vector	NULL	Which HPD levels to swap; NULL = all.
plotCheck	logical	FALSE	Plot tips before/after swap.
________________________________________
5. Visualization / Animation Support
animate_phylomovie(tr, times, outdir="frames", prefix="frame", xlim=c(-180,180), ylim=c(-60,80), propIndex=0)
...
•	Contact / Help:
o	Questions about usage, bug reports, or feature requests can be directed to the maintainer or through your team’s code repository.
________________________________________
